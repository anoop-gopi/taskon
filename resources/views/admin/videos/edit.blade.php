...
<div class="min-h-screen bg-base-100 flex">
  ...
  <div class="flex-1 flex flex-col">
    ...
    <div class="bg-white shadow-sm border-b border-base-300 p-6">
      <h1 class="text-3xl font-bold text-base-content">Edit Video</h1>
      <p class="text-base-content/60 mt-1">Update video details</p>
    </div>
    ...
    <div class="flex-1 overflow-auto p-6">
      <div class="card bg-base-100 shadow-lg max-w-2xl">
        <div class="card-body">
          <form method="POST" action="{{ route('admin.videos.update', $video->id) }}" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div>
              ...
              <input type="text" name="title" value="{{ old('title', $video->title) }}" class="input input-bordered w-full" required>
              @error('title')
            ...
            <div>
              ...
              <textarea name="description" rows="3" class="textarea textarea-bordered w-full">{{ old('description', $video->description) }}</textarea>
              @error('description')
                <span class="text-error text-sm">{{ $message }}</span>
              @enderror
            </div>
            ...
            <div>
              ...
              <textarea name="embed_url" rows="3" class="textarea textarea-bordered w-full font-mono text-xs" required>{{ old('embed_url', $video->embed_url) }}</textarea>
              @error('embed_url')
                <span class="text-error text-sm">{{ $message }}</span>
              @enderror
            </div>
            ...
            <div>
              ...
              <input type="number" name="display_order" value="{{ old('display_order', $video->display_order) }}" class="input input-bordered w-full">
              @error('display_order')
                <span class="text-error text-sm">{{ $message }}</span>
              @enderror
            </div>
            ...
            <div>
              <label class="label cursor-pointer">
                ...
                <input type="checkbox" name="is_active" class="checkbox" {{ old('is_active', $video->is_active) ? 'checked' : '' }}>
              </label>
            </div>
